...
void j1Map::PropagateBFS(int x, int y)
{
	...
	if (frontier.Count() != 0) {
		...
		iPoint neighbours[4];
		neighbours[0] = iPoint(current.x + 1, current.y);
		neighbours[1] = iPoint(current.x, current.y + 1);
		neighbours[2] = iPoint(current.x - 1, current.y);
		neighbours[3] = iPoint(current.x, current.y - 1);
		
		//Add walkable, unvisited neighbours to the frontier
		for (int i = 0; i < 4; i++) {
...
bool j1Map::IsWalkable(int x, int y) const
{
	...
	//find navigation layer
	MapLayer* nav_layer = nullptr;
	p2List_item<MapLayer*>* layer = data.layers.start;
	while (layer != nullptr) {
		if (layer->data->properties.Get("Navigation") == true) {
			nav_layer = layer->data;
			break;
		}
		layer = layer->next;
	}
	if (nav_layer == nullptr) return ret; //Ret false if no nav_layer can be found.

	//Check if x and y are within the map, and if they are on walkable(tile id == 0)
	if ((x < data.width && x >= 0) && (y < data.height && y >= 0) && nav_layer->Get(x, y) == 0) {
		ret = true;
	}
