	...
	for (int y = 0; y < data.height; y++) {
		for (int x = 0; x < data.width; x++) {
			...
			App->render->Blit(data.tilesets[0]->texture,
				...
				&data.tilesets[0]->GetTileRect(Iterator->attribute("gid").as_uint()));
...
iPoint j1Map::WorldToMap(int x, int y) const
{
	...
	if (data.type == MAPTYPE_ISOMETRIC) {
		ret.x = (y + x / 2)/data.tile_height - 1;
		ret.y = (y - x / 2)/data.tile_height;
	}
	return ret;
...
SDL_Rect TileSet::GetTileRect(int id) const
{
	...
	int x = ((id - firstgid) % num_tiles_width);
	int y = ((id - firstgid) / num_tiles_width);

	rect.x = x * tile_width + margin + spacing * x;
	rect.y = y * tile_height + margin + spacing * y;
	rect.w = tile_width;
	rect.h = tile_height;
